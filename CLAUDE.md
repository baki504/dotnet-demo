...
#### 2. Green → Refactor: プロダクションコード実装
- テストを通す実装を書く
- **セルフレビューを行い、納得がいくまで実装とレビューを繰り返す**
  - テストが全てパスしているか確認
  - コードの可読性・設計の妥当性を自己評価
  - 改善点があれば修正し、再度テスト実行
  - 問題がなくなるまでこのサイクルを続ける
...
#### 3. Lint & Format
- `dotnet format` を実行してコードスタイルを統一する
- Roslyn Analyzer の警告があれば対応する

#### 4. 人間レビュー依頼
- 実装結果を人間に提示し、レビューを依頼する
- 人間の承認を得てから次のテストクラスへ進む

### セルフレビュープロンプト
セルフレビュー時は以下のファイルを参照し、該当する観点を全てチェックすること：
- `.review-prompts/common.md` - 共通（命名、SOLID、コード品質、エラーハンドリング、セキュリティ）
- `.review-prompts/backend.md` - バックエンド（DDD、サービス設計、EF Core、リポジトリ）
- `.review-prompts/frontend.md` - フロントエンド（Controller、ViewModel、Razor View、アクセシビリティ）
- `.review-prompts/test.md` - テスト（設計、網羅性、品質、モック、xUnit）
